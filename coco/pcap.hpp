...
struct PacketHeader {
	...
	void setTimestamp(Time timestamp) {
		setTimestamp(timestamp.value * 1000);
	}
};
...
[[nodiscard]] AwaitableCoroutine readPacket(Buffer &buffer, PacketHeader &header, Array<uint8_t> packet, bool &error);

[[nodiscard]] AwaitableCoroutine writeHeader(Buffer &buffer, const pcap::Header &header, bool &error);
[[nodiscard]] AwaitableCoroutine writePacket(Buffer &buffer, const pcap::PacketHeader &header, Array<const uint8_t> packet, bool &error);
